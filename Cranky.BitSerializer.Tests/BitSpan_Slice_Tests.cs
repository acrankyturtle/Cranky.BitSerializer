...
public class BitSpan_Slice_Tests
{
	...
	public void Test(Parameters parameters)
	{
		...
		var actualValue = newSpan.ReadAs<uint>(b => b);
	...
	{
		const int dataLen = 2;
		var baseData = new byte[dataLen] { 170, 85 };

		for (int spanSize = 1; spanSize < dataLen; spanSize++)
		{
			var data = 0b10101010101010101010101010101010;

			for (int length = 1; length < dataLen * 8; length++)
			{
				for (int offset = 0; offset < 8; offset++)
				{
					for (int sliceOffset = 0; sliceOffset < length; sliceOffset++)
					{
